...
import "./base/Ownable.sol";
	...
	/// @title Token Registry - Stores metadata assocaited with ERC20 tokens. 
	/// @author Logan Ryan Golema - <logan@herc.one>
	...
	contract TokenRegistry is Ownable {
		...
		event LogTokenIpfsHash(address indexed token, bytes oldIpfsHash, bytes newIpfsHash);
		...
		/// @param _swarmHash Swam hash of token icon
		function addToken {
			address _token,
			string _name,
			string _symbol, 
		...
		/// @dev Allows owner to modifiy an existing token's name.
		/// @param _token Address of exisiting token
		...
		/// @dev Allows owner to modify an exisiting token's symbol
		/// @param _token Address of exisiting token
		...
		*/
		...
		/// @dev Provides a registed token's address when given the token name
		/// @param _name Name of registed token
		/// @return Token's address
		function getTokenAddressByName(string _name) constant returns (address) {
			return tokenByName[_name];
		}

		/// @dev Provides a registed token's metadata, looked up by address
		/// @param _token Address of registed token
		...
		/// @dev Provides a registed token's metadata, looked up by name
		/// @param _name Name of registed token
		/// @return Token metadata 
		...
		/// @dev Provides a registed token's metadata, looked up by symbol
		/// @param _symbol Symbol of registed token
		/// @return Token metadata
